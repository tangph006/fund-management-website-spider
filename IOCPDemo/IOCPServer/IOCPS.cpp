...
BOOL IOCPS::Startup()
{
    ...
    if ( !m_bServerStarted )
    {
        ...
        */
        ...
        BOOL bRet2 = CreateCompletionPort();
        bRet &= bRet2;
        if( bRet)
        {
            AppendLog("Completion port successfully created.");		
        }
        // Config the listener.. 
        if ( m_nPortNumber>0 )
        {
...
BOOL IOCPS::SetupIOWorkers()
{
    ...
    for(int i=0; i<m_iMaxIOWorkers;i++)
    {
        ...
        else
        {
            ...
            CString msg;
            msg.Format("Error Can not start worker: %s",ErrorCode2Text(WSAGetLastError()));
            AppendLog(msg);
...
CIOCPBuffer* IOCPS::GetNextSendBuffer(ClientContext *pContext,CIOCPBuffer *pBuff)
{
    ...
    if (pBuff!=NULL)
    {
        ...
        {
            CString msg;
            msg.Format("Duplicate Key! in SendbufferMap");
            AppendLog(msg);
            pContext->m_ContextLock.Unlock();
            return NULL;
        }
...
CIOCPBuffer * IOCPS::GetNextReadBuffer(ClientContext *pContext, CIOCPBuffer *pBuff)
{
    ...
    if (pBuff!=NULL)
    {
        ...
        {
            CString msg;
            msg.Format("Duplicate Key! in ReadbufferMap");
            AppendLog(msg);
            pContext->m_ContextLock.Unlock();
            return NULL;
        }
...
void IOCPS::MakeOrderdRead(ClientContext *pContext, CIOCPBuffer *pBuff)
{
    if (pContext!=NULL&&pBuff!=NULL)
    {
        ...
        if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
        {
            ...
            {
                CString msg;
                msg.Format("Disconnect in unread Possible Socket Error: %s",ErrorCode2Text(WSAGetLastError()));
                AppendLog(msg);
            }
            //pContext->m_ContextLock.Unlock();
            ReleaseBuffer(pBuff);
...
/*
* Prepares for Send file Transfer. 
*
...
/*
* Prepares the for File transfer. 
*
* No other type of sends must be made when transferring files.   
*
* 1) The Function opens the specified file.  
* 2) Sends a package (see below) that contains information about the file to the Client. 
* Se below. [size header 4b|type of package 1b|Size of file 4b|.. filename..]. 
...
void IOCPS::AddToFile(ClientContext *pContext, DWORD dwIoSize, CIOCPBuffer *pOverlapBuff)
{
    if (pContext->m_File.m_hFile != NULL)
    {
        ...
        {
            /* 
            We have overlapped file data and package data
            [..file data|Pkg..pkg]
            */
...
*	address. Add the CLient to the list otherwise.  
*  
...
*
*  Returns TRUE if the user is inside the bannedIP list address.
*
*	This function is used with ConnectAcceptCondition and
...
BOOL IOCPS::IsInBannedList(sockaddr_in* pCaller)
{
    ...
    // We don't use ordinary covert IP to string and string compare 
    // FIXME: Whats happens when we have 64bit processor
...
void IOCPS::AddToBanList(SOCKET &Socket)
{
    ...
    CString msg="";
    msg.Format("%s banned because of bad message size.",sIP);
    AppendLog(msg);
    */
    //
    //  Get the incoming connection IPadress
    ...
    if ( nResult != INVALID_SOCKET)
    {
        ...
        CString msg="";
        msg.Format("%s banned because of bad message size.",inet_ntoa(sockAddr.sin_addr));
        AppendLog(msg);
...
/*
When you are developing server application you may want to protect your server 
against SYN attacks.. 
...
Specifically, TCP shortens the required interval between SYN-ACK (connection request acknowledgments) retransmissions. (TCP retransmits SYN-ACKS when they are not answered.) As a result, the allotted number of retransmissions is consumed quicker and the unacknowledgeable connection request is discarded faster.

The SYN attack protection is obtained by setting some values in the windows registry before you 
