...
int CSortListCtrl::AddItem( int nACTuser,LPCTSTR pszText, ... )
{
	...
	VERIFY( SetArray( nACTuser, iIndex, arrpsz,clrText,clrBak ) );

	return iIndex;
}

int CSortListCtrl::AddItem( int nACTuser,CStringArray& strArray)
{
	const int iIndex = InsertItem( GetItemCount(), strArray[0] );
	LPTSTR* arrpsz = new LPTSTR[ m_iNumColumns ];
	COLORREF * clrText = new COLORREF[ m_iNumColumns ];
	COLORREF * clrBak= new COLORREF[ m_iNumColumns ];
	
	arrpsz[ 0 ] = new TCHAR[ lstrlen( strArray[0] ) + 1 ];
	clrText[ 0 ] = crWindowText;
	clrBak[ 0 ] = crWindow;
	(void)lstrcpy( arrpsz[ 0 ], strArray[0] );
...
//		pszText = va_arg( list, LPCTSTR );
//		ASSERT_VALID_STRING( pszText );
		VERIFY( CListCtrl::SetItem( iIndex, iColumn, LVIF_TEXT, strArray[iColumn], 0, 0, 0, 0 ) );
		
		arrpsz[ iColumn ] = new TCHAR[ lstrlen( strArray[iColumn] ) + 1 ];
		clrText[ iColumn ] = crWindowText;
		clrBak[ iColumn ] = crWindow;
		(void)lstrcpy( arrpsz[ iColumn ], strArray[iColumn] );
	}
...
void CSortListCtrl::SaveColumnInfo()
{
	...
	DWORD dwLen = static_cast<DWORD>(memFile.GetLength());
...
BOOL CSortListCtrl::SetArray(int nACTuser, int iItem, LPTSTR* arrpsz,COLORREF * clrText,COLORREF * clrBak )
{
	...
	pid->crText = clrText;
	pid->crBak = clrBak;
	pid->nACTermUser = nACTuser;
	return CListCtrl::SetItemData( iItem, DWORD_PTR( pid ) );//set item data /
}
...
*/
	...
	SetItemData(iIndex,DWORD_PTR(m_pSortItemData) );
