...
BOOL CTreePropSheet::OnInitDialog() 
{
	...
	#if _MFC_VER >= 0x0700
	{
		m_pwndPageTree->CreateEx(
			WS_EX_CLIENTEDGE|WS_EX_NOPARENTNOTIFY, 
			WS_TABSTOP|WS_CHILD|WS_VISIBLE|dwTreeStyle, 
			CRect(0,0,0,0), this, s_unPageTreeId);
	}
	#else
	{
		m_pwndPageTree->CreateEx(
	...
	m_pwndPageTree->SetWindowPos(&CWnd::wndTop,rectTree.left,rectTree.top,rectTree.Width(),rectTree.Height(),SWP_SHOWWINDOW);
...
void CTreePropSheet::OnPageTreeSelChanging(NMHDR *pNotifyStruct, LRESULT *plResult)
{
	...
	*plResult = 0;	
	...
	if (nPage<0 || (unsigned)nPage>=m_pwndPageTree->GetCount())	
	{
		...
		if (NULL == pTvn->itemOld.hItem)
			bIsParent = FALSE;		
		else
		{
			hItem = m_pwndPageTree->GetParentItem (pTvn->itemOld.hItem);			
			while (hItem)
			{
				if (hItem == pTvn->itemNew.hItem)
				{
					bIsParent = TRUE;
					break;
				}
		...
		{
			if (m_pwndPageTree->ItemHasChildren (pTvn->itemNew.hItem))
			{
				hChildItem = m_pwndPageTree->GetChildItem (pTvn->itemNew.hItem);				
				while (hChildItem)
				{
					hNextItem = m_pwndPageTree->GetNextItem (hChildItem, TVGN_NEXT);
					nSecondPage = (int)m_pwndPageTree->GetItemData (hChildItem);
					if (nSecondPage >= 0 && nSecondPage < (int)m_pwndPageTree->GetCount ())
					{
						break;					
					}
					else
