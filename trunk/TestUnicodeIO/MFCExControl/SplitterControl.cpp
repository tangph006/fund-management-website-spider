...
// 	CBrush* pOB = dc.SelectObject(CBrush::FromHandle((HBRUSH)GetStockObject(NULL_BRUSH)));
// 	CPen* pOP = dc.SelectObject(CPen::FromHandle((HPEN)GetStockObject(NULL_PEN)));
// 	
// 	dc.Rectangle(rcClient);
// 
// 	dc.SelectObject(pOB);
...
void CSplitterControl::GetRange(int &nMin, int &nMax)
{
	nMin=m_nMin;
	nMax=m_nMax;
}

void CSplitterControl::SetBuddies( CWnd* pTopLeft, CWnd* pBottomRight )
{
	CRect rcTopLeft;
	pTopLeft->GetWindowRect(rcTopLeft);
	CRect rcBottomRight;
	pBottomRight->GetWindowRect(rcBottomRight);

	CRect rcTo;
	if (SPS_VERTICAL == m_nType)
	{
		rcTo.left = rcTopLeft.right;
		rcTo.top = min(rcTopLeft.top, rcBottomRight.top);
		rcTo.right = rcBottomRight.left;
		rcTo.bottom = max(rcTopLeft.bottom, rcBottomRight.bottom);
	}
	else if (SPS_HORIZONTAL == m_nType)
	{
		rcTo.left = min(rcTopLeft.left, rcBottomRight.left);
		rcTo.top = rcTopLeft.bottom;
		rcTo.right = max(rcTopLeft.right, rcBottomRight.right);
		rcTo.bottom = rcBottomRight.top;
	}
	else
	{
		ASSERT(FALSE);
	...
	GetParent()->ScreenToClient(rcTo);
	SetWindowPos(NULL,rcTo.left,rcTo.top,rcTo.Width(),rcTo.Height(),SWP_NOACTIVATE|SWP_NOZORDER);
}