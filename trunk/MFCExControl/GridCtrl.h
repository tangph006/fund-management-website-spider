...
enum CellDataType { GridNULL, GridINT, GridSTRING, GridDATETIME, GridDEC ,GridTime};
...
class CGridCtrl;
void MFCEXCONTROL_API AFXAPI DDX_GridControl(CDataExchange* pDX, int nIDC, CGridCtrl& rControl);

/////////////////////////////////////////////////////////////////////////////
// CGridCtrl window

class MFCEXCONTROL_API CGridCtrl : public CWnd
{
...
public:
	...
	void SetDataType(int iItem, CellDataType Type) { m_arrColType[iItem] = Type; }
	int GetVirtualColumnCount();
	int GetVirtualRowCount();
	void SetFlags(const std::vector<bool>& vFlags);

	void SetEnableNoScroll(BOOL bEnable) { m_bEnableScroll = bEnable;}
	...
	UINT GetClipFormat()const {return m_nDragFormat;}
...
public:
    ...
    void ExpandToFit();
	void SetColumnToResize(BOOL bCol);
	BOOL GetColumnToResize()const;
	...
	void SortColumn( int p_nSubItem, int nSortingMultipleColumns = 0 );
